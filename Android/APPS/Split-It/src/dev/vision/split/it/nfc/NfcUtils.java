package dev.vision.split.it.nfc;
 ...
 */
@TargetApi(Build.VERSION_CODES.GINGERBREAD_MR1) public class NfcUtils {
	...
	public static boolean writeTag(NdefMessage message, Tag tag) {
		...
		try {
			...
			if (ndef != null) {
				ndef.connect();
				if (!ndef.isWritable()) {
					Log.e(getLogTag(), "Not writing to tag- tag is not writable");
					return false;
				}
				if (ndef.getMaxSize() < size) {
					Log.e(getLogTag(), "Not writing to tag- message exceeds the max tag size of " + ndef.getMaxSize());
					return false;
				}
				...
				if (format != null) {
					try {
						...
						Log.e(getLogTag(), "Not writing to tag", e);
						return false;
					}
				} else {
					Log.e(getLogTag(), "Not writing to tag- undefined format");
					return false;
				}
			}
		} catch (Exception e) {
	...
	public static List<NdefMessage> getMessagesFromIntent(Intent intent) {
		...
		if (NfcAdapter.ACTION_TAG_DISCOVERED.equals(action) || NfcAdapter.ACTION_NDEF_DISCOVERED.equals(action)) {
			Log.i(getLogTag(), "Reading from NFC " + action);
